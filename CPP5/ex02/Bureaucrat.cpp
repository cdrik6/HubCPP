...
void Bureaucrat::signForm(AForm& doc) const
{
    ...
    {
        std::cerr << _name << " couldn't sign " << doc.getName();
        std::cerr << " because " << e.what() << std::endl; //e of Form::GradeTooLowException
    }
}
...
void Bureaucrat::executeForm(AForm const& doc) const
{
    ...
    {
        doc.execute(*this);            
        std::cout << _name << " executed " << doc.getName() << std::endl;
    }
    catch(std::exception& e)
    {
        std::cout << _name << " couldn't execute " << doc.getName();
